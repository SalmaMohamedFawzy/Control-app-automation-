...
public class HomePage {
    ...
    public void waitHubMenuVisiblityANDassertThatHubIsTEST_A1(WebDriver driver) {
        ...
        try {
            Assert.assertEquals(selectedHub, "TEST-A1", "The selected Hub is not 'TEST-A1'.");
    ...
    public void AssertThatServiceIsB2CDelivery(WebDriver driver) {
        ...
        try {
            Assert.assertTrue(selectedOption.contains("B2C"));
        } catch (AssertionError e) {
    ...
    {
        Assert.assertEquals(selectedOptionText, "Pending", "The selected dropdown option is not 'Pending'.");
    } catch(
    AssertionError e)
